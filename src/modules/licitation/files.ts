...
const uploadsRoot = path.join(process.cwd(), "public", "uploads", "licitations");
...
function getPublicPathSegments(relativePath: string) {
  ...
  if (!normalized.startsWith("/uploads/licitations/")) {
    throw new Error("Caminho de arquivo inválido.");
  }

  return normalized.replace("/uploads/licitations/", "").split("/");
}

export async function saveLicitationPdf(
  file: File,
  options: {
    scope: "edital" | "homologation";
    prefix: string;
  },
) {
  ensurePdfFile(file);
  ...
  return {
    ...
    filePath: `/uploads/licitations/${options.scope}/${fileName}`,
...
export async function deleteStoredLicitationFile(filePath?: string | null) {
  if (!filePath) {
    return;
  }
